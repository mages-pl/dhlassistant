	...
	SendToParcelShopChangePlCarrierPop = function()
	{
		...
		if (val)
		{
			...
			//$('#dhlassistant_pl_checkbox').parent().parent().addClass('disabled');
			$('#dhlassistant_parcel_ident').show();
			// if (dhlassistant_is_map_for_parcel_available)
	...
	{
		if (parseInt($("#PL_CARRIER_POP_dhlassistant_carrier_id").val()) !== parseInt($(".delivery_option_radio input[type='radio']:checked").val())) {
			return;
		}
