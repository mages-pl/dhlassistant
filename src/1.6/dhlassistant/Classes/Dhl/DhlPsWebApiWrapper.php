...
// 		if ($oShipment->HasLabelFile())
// 			unlink (Wrappers\ConfigWrapper::Get('LabelsDir').$oShipment->GetLabelFileName());
...
// 		$file_name = $oShipment->GetLabelFileName($sLabelType);
// 		file_put_contents(Wrappers\ConfigWrapper::Get('LabelsDir').$file_name, base64_decode($result->getLabelResult->labelContent));
// 		return $file_name;
		return base64_decode($result->getLabelResult->labelContent);
	}
	public static function GetPnpReport(DataModels\DhlUser $oDhlUser, \DateTime $oDate, $aParams = null)
	{
		/**
...
// 			throw new Exceptions\LoggedException("Usługa specjalna 'COD' nie jest dostępna przy wysyłce do Parcelstation w usłudze '{$dcs->GetName()}' w kraju {$oShipment->GetTargetCountry()->Name}!");
	}
