...
//            || Wrappers\SourceWrapper::InstallCarrier() == false
            ...
            || $this->registerHook('displayCarrierList') == false
    ...
    public function getContent()
    {
        ...
        } else if ($page == 'Edit' && ($id = Tools::getValue('id'))) {
            $iframe_link = (new Controllers\ShipmentEdit())->GetLink(array('source_id' => $id));
        } else if (class_exists(($class_name = '\DhlAssistant\Classes\Controllers\\' . $page))) {
    ...
    public function hookDisplayAdminOrder($aParams)
    {
        if (!Wrappers\SourceWrapper::HasUserUsingRight() || !isset($aParams['id_order'])) {
            return '';
        }
        ...
        if (!Wrappers\ConfigWrapper::Get('IsModuleConfigured')) {
            ...
            $source_id = (int)$aParams['id_order'];
    ...
    public function hookDisplayCarrierList($aParams)
    {
        try {
            $cart = context::getContext()->cart;
            $carrier_id = $cart->id_carrier;
            $carrierCode = DbWrapper::GetCodeCarrier($carrier_id);
            if (!in_array($carrierCode, ['PL_CARRIER_POP', 'PL_CARRIER_POP_COD', 'FOREIGN_CARRIER_PARCELSHOP'])) {
                return '';
            }

            $aParams['address'] = current($this->context->cart->getAddressCollection());
            $aParams['cart'] = $this->context->cart;

            $country = new Country((int)$aParams['address']->id_country);
            ...
            $smarty_params = [
                ...
                'dhlassistant_carrier_id' => $carrier_id,
    ...
    public function hookDisplayPaymentTop($aParams)
    {
        try {
            if (!Wrappers\ConfigWrapper::Get('IsModuleConfigured')
                || !\Validate::isLoadedObject($aParams['cart'])
                || !($aParams['cart']->id_carrier == Wrappers\ConfigWrapper::Get('CarrierId'))
            ) {
                return '';
            }

            $address = new \Address ((int)$aParams['cart']->id_address_delivery);
            ...
            $shop = new \Shop($aParams['cart']->id_shop);
    ...
    {
        if (!isset($aParams['id_carrier']) 
		|| !isset($aParams['carrier']) 
		|| !\Validate::isLoadedObject($aParams['carrier'])) {
            return;
	}

        $oldCarrierId = (int)$aParams['id_carrier'];
        $newCarrierId = (int)$aParams['carrier']->id;
